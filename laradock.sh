...
if [[ $type == 'up' ]]; then
 cd ${PWD}/../laradock && docker-compose --env-file .env.laravel-inertia-react up -d $container
 ...
 cd ${PWD}/../laradock && docker-compose --env-file .env.laravel-inertia-react down $container
 cd ${PWD}/../laradock && docker-compose --env-file .env.laravel-inertia-react build $container --no-cache
 cd ${PWD}/../laradock && docker-compose --env-file .env.laravel-inertia-react up -d $container
fi
